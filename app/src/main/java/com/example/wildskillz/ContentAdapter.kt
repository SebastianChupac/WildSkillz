...
class ContentAdapter(
    ...
    inner class ContentViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        val actionButton = view.findViewById<Button>(R.id.btnAction)
    ...
    override fun onBindViewHolder(holder: ContentViewHolder, position: Int) {
        ...
        when (content) {
            is LearningContent.Video -> {
                ...
                holder.actionButton.text = "Watch >"
                holder.actionButton.backgroundTintList = ContextCompat.getColorStateList(context, R.color.tertiary)
            }
            is LearningContent.Workshop -> {
                holder.details.text = "${content.author} • ${content.time} @ ${content.location}"
                holder.actionButton.text = "Attend +"
                holder.actionButton.backgroundTintList = ContextCompat.getColorStateList(context, R.color.teach_light)
            }
        }

        holder.actionButton.setOnClickListener { onClick(content) }
    }
